...
fn sign_and_submit(
    ...
    let msg = match bincode::serialize(&tx) {
        Ok(b) => b,
...
async fn submit_transaction(
    ...
    let msg = bincode::serialize(&tx).unwrap_or_default();
...
async fn wallet_new() -> Json<WalletNewResponse> {
    match SigningKeyPair::generate() {
        Ok(kp) => {
            ...
            let pkcs8_hex = bincode::serialize(&kp).map(hex::encode).unwrap_or_default();
...
async fn wallet_send(
    ...
    let kp: SigningKeyPair = match bincode::deserialize(&pkcs8_bytes) {
        Ok(k) => k,
...
async fn dex_swap(
    ...
    let kp: redflag_crypto::SigningKeyPair = match bincode::deserialize(&pkcs8_bytes) {
        Ok(k) => k,
