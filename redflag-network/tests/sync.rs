...
async fn test_dag_sync_parent_fetching() -> Result<(), Box<dyn std::error::Error>> {
    ...
    node_a.broadcast_message(NetworkMessage::NewBlock(postcard::to_allocvec(&v2).unwrap())).await?;
        ...
        tokio::select! {
            ...
            event_b = node_b.next_event() => {
                if let Some(libp2p::swarm::SwarmEvent::Behaviour(RedFlagBehaviourEvent::DagSync(e))) = event_b {
                    ...
                    if let Ok(NetworkMessage::NewBlock(data)) = postcard::from_bytes::<NetworkMessage>(&message.data) {
                        if let Ok(vertex) = postcard::from_bytes::<Vertex>(&data) {
